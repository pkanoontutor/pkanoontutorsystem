...
from .models import (
    Student, School, TutoringClass, Subject, Sheet,
    ClassSubject, Enrollment, Attendance, EnrollmentInstallment, SheetInventory,
...
class StudentAdmin(admin.ModelAdmin):
    ...
    list_display = ("student_code", "nickname", "full_name", "grade_level", "school_display", "profile_image_thumb", "parent_phone", "is_active")
    search_fields = ("student_code", "nickname", "full_name", "school__name", "parent_phone")
    list_filter = ("grade_level", "is_active", "contact_channel", "referral_source")
    list_per_page = 50
    readonly_fields = ("student_code", "profile_image_thumb_large", "created_at")
    fieldsets = (
        ("รูปโปรไฟล์", {"fields": ("profile_image", "profile_image_thumb_large")}),
        ("ข้อมูลนักเรียน", {"fields": ("student_code", "nickname", "full_name", "grade_level", "academic_year", "school", "parent_phone", "is_active")}),
        ("ข้อมูลการติดต่อ", {"fields": ("contact_channel", "referral_source", "enroll_date")}),
        ("หมายเหตุ", {"fields": ("note",)}),
        ("ระบบ", {"fields": ("created_at",)}),
    )
...
class ClassSubjectInline(admin.TabularInline):
    ...
    fields = ("subject", "current_sheet", "current_page", "current_question", "last_teacher", "is_active", "updated_at", "updated_by")
...
class ClassSubjectAdmin(admin.ModelAdmin):
    list_display = ("tutoring_class", "subject", "current_sheet", "current_page", "current_question", "last_teacher", "is_active", "updated_at")
    list_filter = ("tutoring_class", "subject", "is_active")
    search_fields = ("tutoring_class__name", "subject__name", "current_sheet__code", "current_sheet__title")
...
class EnrollmentAdmin(admin.ModelAdmin):
    ...
    list_display = ("sale_run_no", "student", "tutoring_class", "sessions_total", "is_active", "payment_type", "course_price", "net_price", "created_at")
    list_filter = ("tutoring_class", "is_active", "payment_type")
    search_fields = ("sale_run_no", "student__student_code", "student__full_name", "student__nickname", "tutoring_class__name")
    readonly_fields = ("sale_run_no", "created_at", "net_price")
    fieldsets = (
        ("ข้อมูลนักเรียน", {"fields": ("student", "tutoring_class", "sale_run_no", "created_at", "is_active")}),
        ("จำนวนครั้ง", {"fields": ("sessions_total", "remark")}),
...
class SheetInventoryAdmin(admin.ModelAdmin):
    list_display = ("sheet", "quantity", "is_finished", "updated_at")
...
class QuizAdmin(admin.ModelAdmin):
    list_display = ("title", "subject_name", "grade_level", "total_questions", "total_score", "time_limit_minutes", "pass_score", "is_active", "created_at")
    list_filter = ("grade_level", "subject_name", "is_active")
    search_fields = ("title", "subject_name")
    inlines = (QuestionInline,)
    readonly_fields = ("created_at",)
    fieldsets = (
        ("ข้อมูลชุดข้อสอบ", {"fields": ("title", "subject_name", "grade_level", "description", "is_active", "created_at")}),
        ("กฎการสอบ", {"fields": ("time_limit_minutes", "pass_score")}),
    )
...
class QuestionAdmin(admin.ModelAdmin):
    ...
    list_filter = ("quiz__grade_level", "question_type")
...
class QuizAttemptAdmin(admin.ModelAdmin):
    list_display = ("taker_nickname", "taker_firstname", "taker_lastname", "taker_grade", "quiz", "status", "score", "max_score", "score_pct", "passed", "submitted_at")
    list_filter = ("taker_grade", "status", "passed", "quiz__subject_name")
    search_fields = ("taker_nickname", "taker_firstname", "taker_lastname", "quiz__title")
    readonly_fields = ("taker_nickname", "taker_firstname", "taker_lastname", "taker_grade", "taker_school", "taker_email", "quiz", "status", "score", "max_score", "passed", "started_at", "submitted_at", "session_key")
